...
import com.moaazelneshawy.kmm.translatorapp.voiceToText.presentation.DisplayState
import com.moaazelneshawy.kmm.translatorapp.voiceToText.presentation.VoiceToTextEvent
import com.moaazelneshawy.kmm.translatorapp.voiceToText.presentation.VoiceToTextState
                ...
                VoiceToTextEvent.PermissionResult(
                    ...
                    isPermanentlyDeclined = !isGranted &&
                            (context as ComponentActivity)
                                .shouldShowRequestPermissionRationale(audioPermission).not()
                )
            )
        }
    )
    // will execute once when the screen is open
    LaunchedEffect(recordAudioPermissionLauncher) {
        recordAudioPermissionLauncher.launch(audioPermission)
    }
    Scaffold(
                ...
                AnimatedContent(targetState = state.displayState) { displayState ->
                    when (displayState) {
                        DisplayState.WAITING_TO_TALK -> {
                            Text(
                                text = stringResource(id = R.string.waiting_to_talk),
